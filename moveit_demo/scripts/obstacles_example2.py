...
class CollisionSceneExample(object):
    ...
    def add_two_box(self):
        box1_pose = [0.25, 0.25, 0.0, 0, 0, 0, 1]
        box1_dimensions = [0.25, 0.75, 0.50]

        box2_pose = [-0.15, 0.30, 0.0, 0, 0, 0, 1]
        box2_dimensions = [0.25, 0.30, 0.75]

        self.add_box_object("box1", box1_dimensions, box1_pose)
        self.add_box_object("box2", box2_dimensions, box2_pose)

        print("============ Added two obstacle to RViz!!")
    ...
    def add_one_sphere(self):
        sphere1_pose = [0.60, -0.20, 0.1, 0, 0, 0, 1]
        sphere1_radi = 0.27

        self.add_sphere_object("sphere1", sphere1_pose, sphere1_radi)

        print("============ Added one obstacle to RViz!!")

    def add_one_cylinder(self):
        cylinder1_pose = [0.60, -0.20, 0.1, 0, 0, 0, 1]
        cylinder1_height = 0.80
        cylinder1_radi = 0.27

        self.add_cylinder_object("cylinder1", cylinder1_pose, cylinder1_height, cylinder1_radi)

        print("============ Added one obstacle to RViz!!")

    def add_three_objects(self):
        cylinder1_pose = [0.65, -0.20, 0.1, 0, 0, 0, 1]
        cylinder1_height = 0.80
        cylinder1_radi = 0.27

        box1_pose = [0.35, 0.60, 0, 0, 0, 0, 1]
        box1_dimensions = [0.25, 0.25, 1.75]

        box2_pose = [0.70, 0.30, 0.14, 0, 0, 0, 1]
        box2_dimensions = [0.28, 0.28, 0.22]
        ...
        print("========== Added 3 obstacles to the scene!!")
    ...
    def add_sphere_object(self, name, pose, radi):
        p = geometry_msgs.msg.PoseStamped()
        p.header.frame_id = self.robot.get_planning_frame()
        p.header.stamp = rospy.Time.now()
        p.pose.position.x = pose[0]
        ...
        self.scene.add_sphere(name, p, radi)

    def add_cylinder_object(self, name, pose, height, radi):
        p = geometry_msgs.msg.PoseStamped()
        p.header.frame_id = self.robot.get_planning_frame()
        p.header.stamp = rospy.Time.now()
        p.pose.position.x = pose[0]
        ...
        self.scene.add_cylinder(name, p, height, radi)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "cluttered":
        ...
        load_scene.add_two_box()
    elif sys.argv[1] == "sphere":
        load_scene.add_one_sphere()
    elif sys.argv[1] == "cylinder":
        load_scene.add_one_cylinder()
    elif sys.argv[1] == "cluttered2":
        load_scene.add_three_objects()
    else:
        print("Please specify correct type of scene as cluttered or sparse or double")
        sys.exit()
